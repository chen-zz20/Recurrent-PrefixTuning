...
def angle_defn(pos, i, d_model_size):
    angle_rates = 1 / jt.pow(10000, (2 * (i // 2)) / d_model_size)
...
def positional_encoding(position, d_model_size, dtype):
    ...
    angle_rads = angle_defn(
        jt.arange(position, dtype=dtype).unsqueeze(1),
        jt.arange(d_model_size, dtype=dtype).unsqueeze(0),
        d_model_size,
    )

    sines = jt.sin(angle_rads[:, 0::2])
    cosines = jt.cos(angle_rads[:, 1::2])

    pos_encoding = jt.concat([sines, cosines], dim=-1)
    return pos_encoding


def scaled_dot_product_attention(q, k, v, mask, attention_mask=None, head_mask=None):
    # calculate attention
    matmul_qk = jt.matmul(q, k.permute(0, 1, 3, 2))
    ...
    output = jt.matmul(attention_weights, v)
...
class MultiHeadAttention(jt.nn.Module):
    def __init__(self, d_model_size, num_heads):
        ...
        self.Wq = jt.nn.Linear(d_model_size, d_model_size)
        self.Wk = jt.nn.Linear(d_model_size, d_model_size)
        self.Wv = jt.nn.Linear(d_model_size, d_model_size)

        self.dense = jt.nn.Linear(d_model_size, d_model_size)
    ...
    def forward(
        ...
        if layer_past is not None:
            ...
            k = jt.concat((past_key, k), dim=-2)
            v = jt.concat((past_value, v), dim=-2)

        if use_cache is True:
            present = jt.stack((k, v))
        else:
            present = (None,)
...
def point_wise_feed_forward_network(d_model_size, dff):
    return jt.nn.Sequential(jt.nn.Linear(d_model_size, dff), jt.nn.ReLU(), jt.nn.Linear(dff, d_model_size))


class EncoderLayer(jt.nn.Module):
    def __init__(self, d_model_size, num_heads, dff, rate=0.1):
        ...
        self.layernorm1 = jt.nn.LayerNorm(d_model_size, eps=1e-6)
        self.layernorm2 = jt.nn.LayerNorm(d_model_size, eps=1e-6)

        self.dropout1 = jt.nn.Dropout(rate)
        self.dropout2 = jt.nn.Dropout(rate)
...
class CTRLModel(CTRLPreTrainedModel):
    ...
    def forward(
        ...
        **kwargs,
        ...
        mask = jt.triu(jt.ones(seq_len + past_length, seq_len + past_length), 1)
