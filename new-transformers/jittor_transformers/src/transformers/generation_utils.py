...
class GenerationMixin:
    ...
    def generate(
        ...
        **model_kwargs
        ...
        Parameters:
            ...
            input_ids (:obj:`jt.Var` of shape :obj:`(batch_size, sequence_length)`, `optional`):
                The sequence used as a prompt for the generation. If :obj:`None` the method initializes
                it as an empty :obj:`jt.Var` of shape :obj:`(1,)`.
            ...
            attention_mask (:obj:`jt.Var` of shape :obj:`(batch_size, sequence_length)`, `optional`):
        ...
        Return:
            ...
            :obj:`jt.Var` of shape :obj:`(batch_size * num_return_sequences, sequence_length)`:
        ...
        if (attention_mask is None) and (pad_token_id is not None) and (pad_token_id in input_ids):
            ...
            attention_mask = jt.ones(input_ids.shape)
        ...
        if self.config.is_encoder_decoder:
            ...
            encoder_outputs["last_hidden_state"] = encoder_outputs.last_hidden_state[(slice(),)*0+expanded_batch_idxs]
    ...
    def _generate_no_beam_search(
        ...
        while cur_len < max_length:
            ...
            if do_sample:
                ...
                # Sample
                probs = jt.nn.softmax(next_token_logscores, dim=-1)
                token_distri = jt.distributions.Categorical(probs=probs)
                next_token = token_distri.sample(sample_shape = [1]).squeeze(1)
            ...
            if self.config.is_encoder_decoder is False:
                attention_mask = jt.concat(
                    [attention_mask, jt.ones((attention_mask.shape[0], 1))], dim=-1
                )
    ...
    def _generate_beam_search(
        ...
        while cur_len < max_length:
            ...
            scores = jt.nn.log_softmax(next_token_logits, dim=-1)  # (batch_size * num_beams, vocab_size)
            ...
            if do_sample:
                ...
                probs = jt.nn.softmax(_scores, dim=-1)
                token_distri = jt.distributions.Categorical(probs=probs)
                next_tokens = token_distri.sample(sample_shape = [2 * num_beams])
                # next_tokens = torch.multinomial(probs, num_samples=2 * num_beams)  # (batch_size, num_beams * 2)
                # Compute next scores
                next_scores = jt.gather(_scores, -1, next_tokens)  # (batch_size, num_beams * 2)
                # sort the sampled vector to make sure that the first num_beams samples are the best
                next_scores_indices,next_scores= jt.argsort(next_scores, descending=True, dim=1)
            ...
            beam_scores = jt.stack([x[0] for x in next_batch_beam])
            beam_tokens = jt.stack([x[1] for x in next_batch_beam])
            beam_idx = jt.stack([x[2] for x in next_batch_beam])
    ...
    @staticmethod
    def _reorder_cache(past: Tuple, beam_idx: Var) -> Tuple[Var]:
        return tuple(layer_past[:,beam_idx,...] for layer_past in past)
        return tuple(layer_past.index_select(1, beam_idx) for layer_past in past)

def calc_banned_ngram_tokens(prev_input_ids: Var, num_hypos: int, no_repeat_ngram_size: int, cur_len: int) -> None:
...
def set_scores_to_inf_for_banned_tokens(scores: jt.Var, banned_tokens: List[List[int]]) -> None:
    ...
    if not banned_mask_list:
        return
    banned_mask = jt.Var(banned_mask_list)
    ...
    banned_mask = jt.sparse.SparseVar(banned_mask.t(), indices, scores.size()).to_dense().bool()
