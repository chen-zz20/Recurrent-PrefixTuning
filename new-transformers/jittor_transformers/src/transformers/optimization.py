...
class AdamW(Optimizer):
    ...
    def __init__(
        self,
        params: Iterable[jt.nn.Parameter],
        lr: float = 1e-3,
        ...
        super().__init__(params, defaults["lr"])
...
class Adafactor(Optimizer):
    ...
    *warmup_init* options. To use a manual (external) learning rate schedule you should set `scale_parameter=False` and `relative_step=False`.
    ...
    def _approx_sq_grad(exp_avg_sq_row, exp_avg_sq_col):
        ...
        return jt.nn.matmul(r_factor.unsqueeze(-1), c_factor.unsqueeze(0))
        ...
        for group in self.param_groups:
            for p in group["params"]:
                ...
                if len(state) == 0:
                    ...
                    if use_first_moment:
                        # Exponential moving average of gradient values
                        state["exp_avg"] = jt.zeros_like(grad)
                    if factored:
                        state["exp_avg_sq_row"] = jt.zeros(grad_shape[:-1]).to(grad)
                        state["exp_avg_sq_col"] = jt.zeros(grad_shape[:-2] + grad_shape[-1:]).to(grad)
                    else:
                        state["exp_avg_sq"] = jt.zeros_like(grad)
