...
class TextDataset(Dataset):
    ...
    def __getitem__(self, i) -> jt.Var:
        return jt.Var(self.examples[i]).int64()
...
class LineByLineWithWeightTextDataset(Dataset):
    ...
    def __init__(self, tokenizer: PreTrainedTokenizer, file_path: str, block_size: int):
        ...
        discri_score_full = jt.Var(discri_score)
        # just to double check
        print(discri_score_full.exp().sum(dim=1))
        max_values = jt.max(discri_score_full, dim=1).values
        print(max_values.shape)
        thresh = jt.Var([0.8]).log()
        new_discri_score = []
        new_lm_score = []
        new_sents = []
        todel = []
        ...
        idx = jt.max(jt.Var(self.discri_score), dim=1).indices
...
class LineByLineData2TextTextDataset_Sum(Dataset):
    ...
    def __getitem__(self, i):
        return (jt.Var(self.examples[i]).int64(),
                jt.Var(self.labels[i]).int64(),
                jt.Var(self.src_sent[i]).int64(),
                jt.Var(self.tgt_sent[i]).int64(),
                jt.Var(self.src_cat[i]).int64(),
...
class LineByLineSumTextDataset(Dataset):
    ...
    def __getitem__(self, i):
        ...
        # print(tgt_sent)
        # print()
        return (jt.Var(sent).int64(),
                jt.Var(label).int64(),
                jt.Var(src_sent).int64(),
                jt.Var(tgt_sent).int64(),
                )
...
class LineByLineSentimentTextDataset(Dataset):
    ...
    def __getitem__(self, i):
        return (jt.Var(self.examples[i]).int64(),
                jt.Var(self.labels[i]).int64(),
                jt.Var(self.control_code[i]).int64() )
...
class LineByLineLengthTextDataset(Dataset):
    ...
    def __getitem__(self, i):
        return (jt.Var(self.examples[i]).int64(),
                jt.Var(self.labels[i]).int64(),
                jt.Var(self.control_code[i]).int64() if self.prefix_ctrl else None,)
...
class LineByLineEmbMatchTextDataset(Dataset):
    ...
    def __init__(self, tokenizer: PreTrainedTokenizer, file_path: str, block_size: int, num_layer:int=1,
        ...
        print(self.examples[0])
        print(edited_sents[0])
        self.emb = jt.split(full_score, 1) # (1, num_layer, 1024) or (1, 1024)
    ...
    def get_emb(self, sent_lst, word_lst, num_layer, bos_tok, eos_tok):
        ...
        with jt.no_grad():
            ...
            while computed_ < len(sent_lst):
                ...
                cand_idx = jt.Var([i[-1] for i in cand_idx]) #bsz
                # print(cand_idx)
                ...
                outputs = model(input_ids, attention_mask=mask_input, output_hidden_states=True)
                ...
                if num_layer > 1:
                    ...
                    hidden_layer = jt.stack(selected_all_hidden_states, dim=1)
            ...
            full_score = jt.concat(full_score, dim=0)
...
class LineByLineWithSOPTextDataset(Dataset):
        ...
        while i < len(document):
            ...
            if i == len(document) - 1 or current_length >= target_seq_length:
                if current_chunk:
                    ...
                    example = {
                        "input_ids": jt.Var(input_ids).int64(),
                        "token_type_ids": jt.Var(token_type_ids).int64(),
                        "sentence_order_label": jt.Var(0 if is_next else 1).int64(),
                    }
    ...
    def __getitem__(self, i) -> Dict[str, jt.Var]:
