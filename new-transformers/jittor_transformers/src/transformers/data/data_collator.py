...
def pad_sequence(
    sequences: List[jt.Var],
    batch_first: bool = True,
    padding_value: float = 0.0,
) -> jt.Var:
    to_pad_len = 0
    for eachvar in sequences:
        pad_shape = eachvar.shape[1:]
        if(eachvar.shape[0]>to_pad_len):
            to_pad_len = eachvar.shape[0]
    new_seq = []
    for eachvar in sequences:
        if(not eachvar.shape[0]==to_pad_len):
            cur_pad_size = (to_pad_len-eachvar.shape[0],)+pad_shape
            cur_pad_var = jt.full(shape = cur_pad_size,val=padding_value)
            new_seq.append(jt.concat([eachvar,cur_pad_var]))
        else:
            new_seq.append(eachvar)
    return jt.stack(new_seq)
...
"""
DataCollator = NewType("DataCollator", Callable[[List[InputDataClass]], Dict[str, jt.Var]])


def default_data_collator(features: List[InputDataClass]) -> Dict[str, jt.Var]:
    """
    ...
    if "label" in first and first["label"] is not None:
        label = first["label"].item() if isinstance(first["label"], jt.Var) else first["label"]
        dtype = jt.int64 if isinstance(label, int) else jt.float32
        batch["labels"] = jt.Var([f["label"] for f in features], dtype=dtype)
    elif "label_ids" in first and first["label_ids"] is not None:
        if isinstance(first["label_ids"], jt.Var):
            batch["labels"] = jt.stack([f["label_ids"] for f in features])
        else:
            dtype = jt.int64 if type(first["label_ids"][0]) is int else jt.float32
            batch["labels"] = jt.Var([f["label_ids"] for f in features], dtype=dtype)
    ...
    for k, v in first.items():
        if k not in ("label", "label_ids") and v is not None and not isinstance(v, str):
            if isinstance(v, jt.Var):
                batch[k] = jt.stack([f[k] for f in features])
            else:
                batch[k] = jt.Var([f[k] for f in features])
...
class DataCollatorWithPadding:
    ...
    def __call__(self, features: List[Dict[str, Union[List[int], jt.Var]]]) -> Dict[str, jt.Var]:
...
class DataCollatorForLanguageModeling:
    ...
    def _tensorize_batch(
        ...
        if isinstance(examples[0], (list, tuple)):
            examples = [jt.Var(e, dtype=jt.int64) for e in examples]
    ...
    def mask_tokens(self, inputs: jt.Var) -> Tuple[jt.Var, jt.Var]:
        ...
        probability_matrix.masked_fill_(jt.Var(special_tokens_mask, dtype=jt.bool), value=0.0)
        ...
        random_words = jt.randint(len(self.tokenizer), labels.shape, dtype=jt.int64)
...
class DataCollatorForWeightedLanguageModeling:
    ...
    def __call__(
        ...
        # weights_lm = jt.Var(weights_lm)
        weights_discri = jt.Var(weights_discri)
        # print(weights_discri)
        best_lab = jt.max(weights_discri, dim=1)
        # print(best_lab)
        if self.tokenizer.pad_token_id is not None:
            labels[labels == self.tokenizer.pad_token_id] = -100
...
class DataCollatorForEmbMatchLanguageModeling:
    ...
    def __call__(
        ...
        if self.mlm:
            ...
            batch = self._tensorize_batch(input_ids)
            labels = self._tensorize_batch(labels)
            # emb = jt.stack(emb, dim=0)
            emb = jt.concat(emb, dim=0)
            # print(emb.shape)
            # print(batch[0], labels[0])
            if self.tokenizer.pad_token_id is not None:
                labels[labels == self.tokenizer.pad_token_id] = -100
...
class DataCollatorForSumBatchGenLanguageModeling:
    ...
    def __call__(
        ...
        if mode_gen == 0:
            ...
            bos_seq = jt.ones(bsz, 1).fill_(self.tokenizer.bos_token_id).long()
            src_input_ids = jt.concat([src['input_ids'], bos_seq], dim=-1)
            bos_mask = jt.ones(bsz, 1).long()
            src_mask = jt.concat([src["attention_mask"], bos_mask],dim=-1)
...
class DataCollatorForSOP(DataCollatorForLanguageModeling):
    ...
    def __call__(self, examples: List[Dict[str, jt.Var]]) -> Dict[str, jt.Var]:
        ...
        sentence_order_label = jt.stack(sop_label_list)
    ...
    def mask_tokens(self, inputs: jt.Var) -> Tuple[jt.Var, jt.Var, jt.Var]:
        """
...
class DataCollatorForPermutationLanguageModeling:
    ...
    def _tensorize_batch(
        ...
        # In order to accept both lists of lists and lists of Tensors
        if isinstance(examples[0], (list, tuple)):
            examples = [jt.Var(e) for e in examples]
        length_of_first = examples[0].size(0)
        are_tensors_same_length = all(x.size(0) == length_of_first for x in examples)
        if are_tensors_same_length:
            return jt.stack(examples, dim=0)
    ...
    def mask_tokens(self, inputs: jt.Var) -> Tuple[jt.Var, jt.Var, jt.Var, jt.Var]:
        """
        ...
        masked_indices = jt.full(labels.shape, 0, dtype=jt.bool)
        target_mapping = jt.zeros((labels.size(0), labels.size(1), labels.size(1)), dtype=jt.float32)
        ...
        for i in range(labels.size(0)):
            ...
            while cur_len < max_len:
                ...
                span_length = jt.randint(1, self.max_span_length + 1, (1,)).item()
                # Reserve a context of length `context_length = span_length / plm_probability` to surround the span to be masked
                context_length = int(span_length / self.plm_probability)
                # Sample a starting point `start_index` from the interval `[cur_len, cur_len + context_length - span_length]` and mask tokens `start_index:start_index + span_length`
                start_index = cur_len + jt.randint(context_length - span_length + 1, (1,)).item()
            ...
            target_mapping[i] = jt.eye(labels.size(1))

        special_tokens_mask = jt.Var(
            [self.tokenizer.get_special_tokens_mask(val, already_has_special_tokens=True) for val in labels.tolist()],
            dtype=jt.bool,
        )
        ...
        perm_mask = jt.zeros((labels.size(0), labels.size(1), labels.size(1)), dtype=jt.float32)
        ...
        for i in range(labels.size(0)):
            ...
            perm_index = jt.arange(labels.size(1))
            # Split this into two halves, assuming that half the sequence is reused each time
            perm_index = perm_index.reshape((-1, labels.size(1) // 2)).transpose(0, 1)
            # Permute the two halves such that they do not cross over
            perm_index = perm_index[jt.misc.randperm(labels.size(1) // 2)]
            # Flatten this out into the desired permuted factorisation order
            perm_index = jt.flatten(perm_index.transpose(0, 1))
            # Set the permutation indices of non-masked (non-functional) tokens to the
            # smallest index (-1) so that:
            # (1) They can be seen by all other positions
            # (2) They cannot see masked ositions, so there won't be information leak
...
class DataCollatorForNextSentencePrediction:
    ...
    def __call__(self, examples: List[Dict[str, jt.Var]]) -> Dict[str, jt.Var]:
        ...
        result = {
            ...
            "next_sentence_label": jt.Var(nsp_labels),
        }
        if self.mlm:
            result["masked_lm_labels"] = mlm_labels
        return result

    def _tensorize_batch(self, examples: List[jt.Var]) -> jt.Var:
        length_of_first = examples[0].size(0)
        are_tensors_same_length = all(x.size(0) == length_of_first for x in examples)
        if are_tensors_same_length:
            return jt.stack(examples, dim=0)
    ...
    def create_features_from_example(self, tokens_a, tokens_b):
        ...
        input_id = jt.Var(input_id)
        attention_mask = jt.Var(attention_mask)
        segment_id = jt.Var(segment_id)
