...
class Attention(nn.Module):
    ...
    def _attn(self, q, k, v, attention_mask=None, head_mask=None, output_attentions=False):
        ...
        if not self.is_cross_attention:
            ...
            w = jt.where(mask.bool(), w, self.masked_bias.to(w.dtype))
    ...
    def forward(
        ...
        **kwargs,
        ...
        if position_ids is None:
            ...
            position_ids = jt.arange(past_length, input_shape[-1] + past_length, dtype=jt.int64, device=device)
    ...
    def forward(
        ...
        **kwargs,
        ...
        if labels is not None and weights is not None:
            ...
            # Flatten the tokens
            loss_fct = jt.nn.cross_entropy_loss
            # print(weights)
            bsz, seqlen, vocab_size = shift_logits.shape
            loss = loss_fct(shift_logits.view(-1, shift_logits.size(-1)), shift_labels.view(-1),reduction='none')
            # print(loss.shape)
