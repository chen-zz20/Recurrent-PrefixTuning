...
from jittor.nn import CrossEntropyLoss, cross_entropy_loss
...
class Attention(nn.Module):
    def __init__(self, nx, n_ctx, config, scale=False, is_cross_attention=False):
        ...
        self.bias=jt.tril(jt.ones((n_ctx, n_ctx), dtype=jt.uint8)).view(1, 1, n_ctx, n_ctx)
        self.masked_bias=jt.Var(-1e4)
    ...
    def _attn(self, q, k, v, attention_mask=None, head_mask=None, output_attentions=False):
        ...
        if not self.is_cross_attention:
            ...
            w = jt.where(mask.expand_as(w).bool(), w, self.masked_bias)
        ...
        if attention_mask is not None:
            ...
            #     attention_mask = jt.concat([temp_ones, attention_mask], dim=-1)
            #     # print('sad')
            w = w + attention_mask
...
class GPT2PreTrainedModel(PreTrainedModel):
    ...
    def _init_weights(self, module):
        ...
        if isinstance(module, (nn.Linear, nn.Embedding, Conv1D)):
            ...
            module.bias = jt.zeros_like(module.bias)
            module.weight = jt.full_like(module.weight,val=1.0)
...
class GPT2DoubleHeadsModelOutput(ModelOutput):
    ...
    Args:
        loss (:obj:`jt.Var` of shape :obj:`(1,)`, `optional`, returned when ``labels`` is provided):
            Language modeling loss.
        mc_loss (:obj:`jt.Var` of shape :obj:`(1,)`, `optional`, returned when :obj:`mc_labels` is provided):
            Multiple choice classification loss.
        logits (:obj:`jt.Var` of shape :obj:`(batch_size, num_choices, sequence_length, config.vocab_size)`):
            Prediction scores of the language modeling head (scores for each vocabulary token before SoftMax).
        mc_logits (:obj:`jt.Var` of shape :obj:`(batch_size, num_choices)`):
    ...
    mc_loss: Optional[jt.Var] = None
    logits: jt.Var = None
    mc_logits: jt.Var = None
...
GPT2_INPUTS_DOCSTRING = r"""
    Args:
        ...
        past_key_values (:obj:`List[jt.Var]` of length :obj:`config.n_layers`):
        ...
        head_mask (:obj:`jt.Var` of shape :obj:`(num_heads,)` or :obj:`(num_layers, num_heads)`, `optional`):
        ...
        inputs_embeds (:obj:`jt.Var` of shape :obj:`(batch_size, sequence_length, hidden_size)`, `optional`):
    ...
    def forward(
        ...
        **kwargs,
        ...
        if position_ids is None:
            position_ids = jt.arange(past_length, input_shape[-1] + past_length).int64()
    ...
    def forward(
        ...
        **kwargs,
        ...
        if labels is not None and weights is not None:
            ...
            loss = cross_entropy_loss(shift_logits.view(-1, shift_logits.size(-1)), shift_labels.view(-1), reduction=None)
            # print(loss.shape)
            ...
            # Flatten the tokens
            loss_fct = CrossEntropyLoss()
            loss = cross_entropy_loss(shift_logits.view(-1, shift_logits.size(-1)), shift_labels.view(-1))
