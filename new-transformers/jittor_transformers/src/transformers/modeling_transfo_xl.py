...
class PositionalEmbedding(nn.Module):
    def __init__(self, demb):
        ...
        inv_freq = 1 / (10000 ** (jt.arange(0.0, demb, 2.0) / demb))
        self.inv_freq = inv_freq

    def forward(self, pos_seq, bsz=None):
        sinusoid_inp = jt.multiply(pos_seq.reshape(-1,1), self.inv_freq)
        pos_emb = jt.concat([sinusoid_inp.sin(), sinusoid_inp.cos()], dim=-1)
...
class RelPartialLearnableMultiHeadAttn(nn.Module):
    def __init__(
        ...
        if r_r_bias is None or r_w_bias is None:  # Biases are not shared
            self.r_r_bias = nn.Parameter(jt.Var(self.n_head, self.d_head))
            self.r_w_bias = nn.Parameter(jt.Var(self.n_head, self.d_head))
        else:
    ...
    def _rel_shift(self, x):
        ...
        zero_pad = jt.zeros(zero_pad_shape, device=x.device, dtype=x.dtype)
        x_padded = jt.concat([zero_pad, x], dim=1)
    ...
    def forward(self, w, r, attn_mask=None, mems=None, head_mask=None, output_attentions=False):
        ...
        if mems is not None:
            ...
            w_head_q, w_head_k, w_head_v = jt.chunk(w_heads, 3, dim=-1)
        ...
        AC = jt.linalg.einsum("ibnd,jbnd->ijbn", (rw_head_q, w_head_k))  # qlen x klen x bsz x n_head

        rr_head_q = w_head_q + self.r_r_bias
        BD = jt.linalg.einsum("ibnd,jnd->ijbn", (rr_head_q, r_head_k))  # qlen x klen x bsz x n_head
        ...
        if attn_mask is not None and jt.sum(attn_mask).item():
        ...
        # [qlen x klen x bsz x n_head]
        attn_prob = nn.softmax(attn_score, dim=1)
        ...
        attn_vec = jt.linalg.einsum("ijbn,jbnd->ibnd", (attn_prob, w_head_v))
...
class AdaptiveEmbedding(nn.Module):
    def __init__(self, n_token, d_embed, d_proj, cutoffs, div_val=1, sample_softmax=False):
        ...
        if div_val == 1:
            ...
            if d_proj != d_embed:
                self.emb_projs.append(nn.Parameter(jt.Var(d_proj, d_embed)))
        else:
            for i in range(len(self.cutoffs)):
                l_idx, r_idx = self.cutoff_ends[i], self.cutoff_ends[i + 1]
                d_emb_i = d_embed // (div_val ** i)
                self.emb_layers.append(nn.Embedding(r_idx - l_idx, d_emb_i))
                self.emb_projs.append(nn.Parameter(jt.Var(d_proj, d_emb_i)))

    def forward(self, inp):
        if self.div_val == 1:
            embed = self.emb_layers[0](inp)
            if self.d_proj != self.d_embed:
                embed = nn.Linear(embed, self.emb_projs[0])
        else:
            param = next(self.parameters())
            inp_flat = inp.view(-1)
            emb_flat = jt.zeros([inp_flat.size(0), self.d_proj], dtype=param.dtype, device=param.device)
            for i in range(len(self.cutoffs)):
                l_idx, r_idx = self.cutoff_ends[i], self.cutoff_ends[i + 1]
                ...
                emb_i = nn.Linear(emb_i, self.emb_projs[i])
...
class TransfoXLLMHeadModelOutput(ModelOutput):
    ...
    losses: Optional[jt.Var] = None
    prediction_scores: jt.Var = None
    mems: List[jt.Var] = None
    hidden_states: Optional[Tuple[jt.Var]] = None
    attentions: Optional[Tuple[jt.Var]] = None
...
class TransfoXLModel(TransfoXLPreTrainedModel):
    ...
    def init_mems(self, bsz):
        if self.mem_len > 0:
            ...
            for i in range(self.n_layer):
                empty = jt.zeros(self.mem_len, bsz, self.config.d_model, dtype=param.dtype, device=param.device)
                mems.append(empty)
    ...
    def _update_mems(self, hids, mems, mlen, qlen):
        ...
        with jt.no_grad():
            ...
            for i in range(len(hids)):
                ...
                cat = jt.concat([mems[i], hids[i]], dim=0)
    ...
    def forward(
        ...
        klen = mlen + qlen
        if self.same_length:
            all_ones = word_emb.new_ones((qlen, klen), dtype=jt.uint8)
            mask_len = klen - self.mem_len
            if mask_len > 0:
                mask_shift_len = qlen - mask_len
            else:
                mask_shift_len = qlen
            dec_attn_mask = (jt.triu(all_ones, 1 + mlen) + jt.tril(all_ones, -mask_shift_len))[:, :, None]  # -1
        else:
            dec_attn_mask = jt.triu(word_emb.new_ones((qlen, klen), dtype=jt.uint8), diagonal=1 + mlen)[
                :, :, None
            ]
        ...
        if self.attn_type == 0:  # default
            pos_seq = jt.arange(klen - 1, -1, -1.0, device=word_emb.device, dtype=word_emb.dtype)
            if self.clamp_len > 0:
