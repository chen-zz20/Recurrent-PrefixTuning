...
from transformers import (
    CTRLLMHeadModel, # OK
    CTRLTokenizer,  # no torch
    GPT2LMHeadModel, # OK
    GPT2Tokenizer, # no torch
    OpenAIGPTLMHeadModel, # OK
    OpenAIGPTTokenizer, # no torch
    AutoConfig,  # no torch
    set_seed,  # no torch
    GPT2LMHeadModelAdapter,  # OK
)
import sys, os
...
def main():
    ...
    if args.task_mode == 'data2text':
        ...
        if QUICK_CHECK:
            ...
            if ('lowdata' in args.model_name_or_path) or (args.prefixModel_name_or_path is not None and 'lowdata' in args.prefixModel_name_or_path):
                # test_path = '/u/scr/xlisali/e2e_data/src1_valid.txt' # TODO
                test_path = '../data/e2e_data/src1_valid.txt' # TODO
            else:
                ...
                if args.eval_dataset == 'valid':
                    ...
                    # test_path = '/u/scr/xlisali/e2e_data/src1_test.txt' # TODO
                    test_path = '../data/e2e_data/src1_test.txt' # TODO
            ...
            decode_mode = 'beam'
            curr_dir = os.path.join('../text-generation/', # TODO
                                    args.gen_dir,
                                    '{}_{}_{}'.format(temp, split_file, decode_mode))
            print(curr_dir)
            gold_dir = os.path.join('../text-generation/', # TODO
                                    args.gen_dir,
                                    '{}_{}_{}'.format(temp, split_file,'gold'))
            print(gold_dir)
            write_e2e_corr(prompt_text_lst, prompt_text_dict, gold_dir)
            src_dir = os.path.join('../text-generation/',  # TODO
                                   args.gen_dir,
        ...
        if args.task_mode == 'webnlg':
            if args.eval_dataset == 'valid':
                ...
                test_path = "../data/webnlg_challenge_2017/dev.json" # TODO
            elif args.eval_dataset == 'test':
                # test_path = "/u/scr/xlisali/WebNLG/webnlg-dataset/webnlg_challenge_2017/test.json"
                test_path = "../data/webnlg_challenge_2017/test.json" # TODO
            else:
            ...
            test_path = "/u/scr/xlisali/DART/dart/data/v1.1.1/dart-v1.1.1-full-test.json" # TODO
        ...
        if QUICK_CHECK:
            ...
            curr_dir = os.path.join('../text-generation/', 
                                    args.gen_dir,
                                    '{}_{}_{}'.format(temp, split_file, decode_mode)) # TODO
            print(curr_dir)
            gold_dir = os.path.join('../text-generation/',
                                    args.gen_dir,
                                    '{}_{}_{}'.format(temp, split_file, 'gold')) # TODO
            print(gold_dir)
            write_e2e_corr(prompt_text_pair, prompt_text_dict, gold_dir)
            src_dir = os.path.join('../text-generation/',
                                    args.gen_dir,
                                    '{}_{}_{}'.format(temp, split_file, 'src')) # TODO
    ...
    for prompt_idx, prompt_text in enumerate(prompt_text_lst):
        ...
        if requires_preprocessing:
            ...
            encoded_prompt = jt.int64(prompt_text).unsqueeze(0)
            print(encoded_prompt.shape)
        else:
            prefix = args.prefix if args.prefix else args.padding_text
            encoded_prompt = tokenizer.encode(prefix + prompt_text, add_special_tokens=False, return_tensors="pt") # TODO
        ...
        if args.tuning_mode == 'prefixtune' or args.tuning_mode == 'bothtune':
            if args.task_mode == 'data2text':
                ...
                if mode in ['cat', 'peek', 'nopeek', 'infix']:
                    ...
                    control_code = jt.int64(cc).unsqueeze(0)
    ...
    if args.task_mode == 'data2text':
        ...
        print(out_file_eval, '\n', gold_dir, '\n', curr_dir)
        os.system("python /home/chenzz/bighomework/corpus/e2e-metrics/measure_scores.py "  # TODO
        ...
        # os.system("bash /u/scr/xlisali/DART/dart/evaluation/run_eval_on_webnlg.sh " # TODO
